    ...
    <DashboardLayout>
      <div className="page-section">
        ...
        <div className="stat-grid">
          {[
            ...
            { label: "Circulating Credits", value: stats.totalPoints.toFixed(2), icon: <Binary className="w-4 h-4" />, color: "text-primary" },
            { label: "Signal Fidelity", value: "0.992", icon: <Fingerprint className="w-4 h-4" />, color: "text-slate-900" }
          ].map((stat, i) => (
        ...
        {!isLoading && filteredLeaderboard.length >= 3 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8 items-end">
            ...
            {/* Rank 1 */}
            <Card role="article" aria-label={`Rank 1: ${filteredLeaderboard[0].name}`} className="relative p-6 md:p-10 pb-8 md:pb-12 rounded-[40px] md:rounded-[64px] border-2 border-primary/20 bg-slate-900 text-white shadow-3xl shadow-primary/10 flex flex-col items-center text-center space-y-6 md:space-y-8 order-1 md:order-2 min-h-[220px] md:min-h-[300px] justify-between group overflow-hidden cursor-pointer" onClick={() => { setSelectedUser(filteredLeaderboard[0]); setIsProfileModalOpen(true); }}>
        ...
        <div className="page-section gap-[var(--space-md)]">
          <div className="section-header border-none pb-0 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-xl font-black tracking-tighter text-slate-900 uppercase">Leaderboard Census</h2>
            </div>
            ...
            {isLoading ? (
              ...
              filteredLeaderboard.map((user) => (
                <Card
                  ...
                  className="p-6 rounded-[32px] border-slate-100 hover:border-primary/20 transition-all group overflow-hidden relative"
                >
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pl-2">
                    <div className="flex items-center gap-6 flex-1">
                      ...
                      <div className="flex items-center gap-5">
                        <div className="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-50 flex items-center justify-center shrink-0 group-hover:bg-white group-hover:shadow-xl group-hover:shadow-slate-100 transition-all overflow-hidden p-0.5">
                          <Avatar className="w-full h-full rounded-xl border-2 border-white">
                            <AvatarFallback className="bg-slate-50 text-xs font-black text-slate-400 uppercase">
                              {user.name[0]}
                            </AvatarFallback>
                          </Avatar>
                        </div>
                        <div>
                          ...
                          <div className="flex items-center gap-3">
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest opacity-60">{user.department}</div>
                    ...
                    <div className="flex items-center gap-10 self-end md:self-center">
                      ...
                      <div className="text-right min-w-20 md:min-w-24">
                        <div className="text-2xl font-black tracking-tighter text-slate-900">{user.points.toFixed(2)}</div>
                        <div className="text-[8px] font-bold text-slate-300 uppercase tracking-widest leading-none opacity-60">Total Accrued</div>
                      </div>
                      <Button variant="ghost" size="icon" className="w-11 h-11 rounded-2xl border border-slate-100 hover:bg-slate-50">
      ...
      <Dialog open={isProfileModalOpen} onOpenChange={setIsProfileModalOpen}>
        <DialogContent className="fixed left-[50%] top-[50%] z-[200] w-[95vw] max-w-[42rem] translate-x-[-50%] translate-y-[-50%] bg-white rounded-[40px] shadow-3xl overflow-hidden focus:outline-none">
          ...
          <div className="p-6 md:p-10 overflow-y-auto max-h-[80vh] custom-scrollbar space-y-8">
            {selectedUser && (
              <>
                ...
                <div className="auto-scroll-row px-2">
                  {[
                    ...
                    { label: "Trust Score", value: selectedUser.trustScore?.toFixed(3) || "0.950" },
                ...
                <div className="space-y-8">
                  ...
                  <div className="flex justify-center pt-2">
                    <Button variant="ghost" onClick={() => setIsProfileModalOpen(false)} className="font-black text-[10px] uppercase tracking-widest text-slate-400 hover:text-slate-900 transition-colors">Close Node Profile</Button>
                  </div>
                </div>
              </>
            )}
