...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  const { userEmail, logout, useMockData } = useAuthStore();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  const navItems = [
    { path: "/dashboard", label: "Neural Engine", icon: LayoutDashboard, description: "Active Detection" },
    { path: "/leaderboard", label: "Global Ranking", icon: Trophy, description: "Impact Census" },
    { path: "/events", label: "Audit Ledger", icon: History, description: "Verification Log" },
    { path: "/wallet", label: "Asset Portal", icon: WalletIcon, description: "Balance & Transfer" },
    { path: "/admin", label: "Review Center", icon: Shield, description: "Governance" },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-slate-900 font-sans">
      ...
      <header className="fixed top-0 left-0 right-0 z-[100] h-20 border-b border-border/40 bg-white/80 backdrop-blur-xl flex items-center">
        <div className="container mx-auto px-6 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col leading-none">
              <span className="text-lg font-black tracking-tighter uppercase text-slate-900 italic">Sustainable</span>
              <span className="text-[9px] font-bold text-slate-400 tracking-[0.3em] uppercase">Campus Automation</span>
            </div>
          </Link>

          <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-2 group cursor-help px-3 py-1.5 rounded-full bg-slate-100 border border-slate-200/50 transition-colors hover:bg-slate-200/50">
            ...
            <div className="flex items-center gap-4">
              <div className="hidden lg:flex flex-col items-end leading-tight mr-2">
                <span className="text-xs font-black text-slate-900 italic">{userEmail?.split('@')[0] || "Guest_01"}</span>
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Campus Node</span>
              </div>
              <Button variant="ghost" size="icon" className="w-10 h-10 rounded-xl border border-border/40 hover:bg-slate-100" onClick={() => navigate("/")}>
                  ...
                  {navItems.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          "flex items-center justify-between group px-4 py-4 rounded-3xl transition-all duration-300",
                          isActive
                ...
                <div className="p-8 rounded-[40px] bg-slate-900 text-white relative overflow-hidden group">
                  ...
                  <div className="relative z-10 space-y-6">
                    <div className="space-y-1">
                      ...
                      <div className="text-lg font-black italic">Operating Range</div>
                    </div>
