...
const DetectionViewer = ({ results }: DetectionViewerProps) => {
  ...
  return (
    <div className="space-y-6" role="log" aria-live="polite" aria-label="Live detection stream">
      ...
      <div className="grid gap-4">
        {results.map((detection) => (
          <div
            key={detection.id}
            role="article"
            aria-label={`Detection: ${getActionLabel(detection.action)} at ${detection.location}`}
          ...
          >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pl-2">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="h-1 w-12 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-primary" style={{ width: `${detection.confidence * 100}%` }} aria-hidden="true" />
                      </div>
                      <span className="text-[10px] font-black text-primary">{Math.round(detection.confidence * 100)}%</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="text-right self-end md:self-center bg-slate-50/50 p-4 rounded-2xl border border-slate-100 md:bg-transparent md:p-0 md:border-none">
                <div className="flex items-center gap-2 text-success font-black text-3xl tracking-tighter">
