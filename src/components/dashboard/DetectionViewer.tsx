...
const DetectionViewer = ({ results }: DetectionViewerProps) => {
  ...
  return (
    <div className="space-y-6" role="log" aria-live="polite" aria-label="Live signal stream">
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">
          <Terminal className="w-3.5 h-3.5" /> Live Signal Stream
        </div>
        <div className="flex items-center gap-2 text-[9px] font-black text-success uppercase tracking-widest animate-pulse">
      ...
      <div className="grid gap-4">
        {results.map((detection) => (
          <div
            key={detection.id}
            role="article"
            aria-label={`Signal: ${getActionLabel(detection.action)} at ${detection.location}`}
          ...
          >
            ...
            <div
              ...
              style={{ opacity: 0.2 + detection.fidelity * 0.8 }}
            />
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pl-2">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest opacity-60">Fidelity Index:</span>
                      <div className="h-1 w-12 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-primary" style={{ width: `${detection.fidelity * 100}%` }} aria-hidden="true" />
                      </div>
                      <span className="text-[10px] font-black text-primary">{Math.round(detection.fidelity * 100)}%</span>
                    </div>
                  </div>
                </div>

                <div className="auto-scroll-row items-center gap-x-8 text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-16">
              ...
              <div className="text-right self-end md:self-center bg-slate-50/50 p-4 rounded-2xl border border-slate-100 md:bg-transparent md:p-0 md:border-none">
                <div className={cn(
                  ...
                  detection.creditsEarned >= 0 ? "text-success" : "text-destructive"
                )}>
                  <span className="text-lg opacity-40">
                    {detection.creditsEarned >= 0 ? "+" : ""}
                  </span>
                  {detection.creditsEarned.toFixed(2)}
                  <Coins className="w-5 h-5 opacity-40" />
                </div>
                <div className="text-[9px] font-bold text-slate-300 uppercase tracking-[0.2em] mt-1">CREDITS_ISSUED</div>
              </div>
            </div>
          </div>
        ))}
